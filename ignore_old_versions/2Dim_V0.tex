...
\begin{proposition}\label{canonical-contraction-condition}
    (Joó-Domokos) Given a connected quiver $Q$ the pair $(tQ, \delta_Q)$ is tight if and only if there is no partition $Q_0 = S \sqcup S'$ such that there is at most one arrow from $S$ to $S'$ and there is at most one arrow from $S'$ to $S$.
\end{proposition}
