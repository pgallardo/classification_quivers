...
\emph{symmetric part} $U := m_k \cap (-m_k)$, the largest subring contained in
$m_k$.
\begin{itemize}
\item
...
\begin{definition}
For an field $k$, sub-semiring $m_k \subset k$, a quiver $Q$, and a weight $\theta \in \map(Q_0, k)$, the \emph{set of admissible flows} associated to $Q$ and $\theta$ is
\[
...
Let $G$ be a group, we recall that a non-empty set $X$ is called a torsor if $X$ is equipped with a left (or right) $G$-action, $\cdot: G \times X \to X$, such that it's  free and transitive. 
\begin{lemma}\label{lem:translation}
The $m_k$-submodule $\mathcal{K}_Q := \ker I_Q \cap \map(Q_1, m_k)$ acts on $\Delta(Q, \theta)$ by translation: if $x \in \Delta(Q, \theta)$ and $y \in \mathcal{K}_Q$, then $x + y \in \Delta(Q, \theta)$. Therefore, $\Delta(Q, \theta)$ is either empty or a torsor for $\mathcal{K}_Q$.
\end{lemma}
...
Next, we calculate the delta flow space for the so called zero and one dimension case, i.e, $b_1(Q) \leq 1$. These examples are the base for the general description in the following section. 

\begin{example}[Tree case]\label{tree}
Suppose $Q$ is a tree, so $b_1(Q) = 0$, $\ker I_Q = 0$, and $I_Q$ is injective.
Then $\mathcal{K}_Q = \{0\}$ regardless of $(k, m_k)$, and for any
$\theta \in \operatorname{Im}(I_Q)$ there is a unique flow
$x_\theta := I_Q^{-1}(\theta) \in \map(Q_1, k)$. Hence $\Delta(Q,\theta)$ is the
singleton $\{x_\theta\}$ when $x_\theta(\alpha) \in m_k$ for all
$\alpha \in Q_1$, and is empty otherwise. Spelled out in the three regimes:
\begin{itemize}
    \item \emph{Ordered case:} $\Delta(Q,\theta) = \{x_\theta\}$ if
    $x_\theta(\alpha) \geq 0$ for all $\alpha \in Q_1$, and $\varnothing$
    otherwise.
    \item \emph{Valued case:} $\Delta(Q,\theta) = \{x_\theta\}$ if
    $v(x_\theta(\alpha)) \geq 0$ for all $\alpha \in Q_1$, and $\varnothing$
    otherwise.
    \item \emph{Intermediate case} $(k = \mathbb{R}((t))$,
    $m_k = \rho^{-1}(\mathbb{R}_{\geq 0}))$\textbf{:}
    $\Delta(Q,\theta) = \{x_\theta\}$ if, for all $\alpha \in Q_1$,
    \[
      v\big(x_\theta(\alpha)\big) \geq 0
      \quad\text{and}\quad
      \rho\big(x_\theta(\alpha)\big) = x_\theta(\alpha)(0) \geq 0,
    \]
    i.e.\ each coordinate is integral with non-negative residue, and
...
\]
Now, let focus on the standard examples for the \emph{ordered case}, that is $k = \mathbb{R}$, $m_k = \mathbb{R}_{\geq 0}$: The conditions $u \geq 0$ and $c - u \geq 0$ force $0 \leq u \leq c$. Thus
\[
\Delta(Q, \theta) \;=\;
\begin{cases}
...
\end{cases}
\]
This is a compact line segment, a one-dimensional polytope.

Now, we discuss the standard example of the \emph{valued case},  $k = \mathbb{F}_2((t))$, $m_k = \mathbb{F}_2[[t]]$, $t$-adic valuation. Since $\mathcal{O}_v$ is a ring, $u \in \mathcal{O}_v$ and $c - u \in \mathcal{O}_v$ hold simultaneously if and only if $u \in \mathcal{O}_v$ and $c \in \mathcal{O}_v$. Hence
\[
\Delta(Q, \theta) \;\cong\;
\begin{cases}
...
\end{cases}
\]
This is infinite when nonempty: the module $\mathcal{K}_Q = \ker I_Q \cap \mathcal{O}_v^2 = \{(u,-u) : u \in \mathcal{O}_v\} \cong \mathcal{O}_v$ acts freely by translation.

Finally, we focus on the standard example for the \emph{intermediate case} $k = \mathbb{R}((t))$,
$m_k = \rho^{-1}(\mathbb{R}_{\geq 0}) = \{x \in \mathbb{R}[[t]] : x(0) \geq 0\}$,
with $\rho : \mathbb{R}[[t]] \to \mathbb{R}$ the residue map $\rho(x) = x(0)$.
Membership $u \in m_k$ now means $v(u) \geq 0$ \emph{and} $\rho(u) \geq 0$, and
likewise for $c - u$. For both $u$ and $c - u$ to be integral we need
$c = u + (c-u) \in \mathbb{R}[[t]]$, i.e.\ $v(c) \geq 0$; assuming this, write
$\bar c := \rho(c) \in \mathbb{R}$. The residue conditions become
$\rho(u) \geq 0$ and $\rho(c - u) = \bar c - \rho(u) \geq 0$, i.e.\
$0 \leq \rho(u) \leq \bar c$, while the higher-order part
$u - \rho(u) \in t\,\mathbb{R}[[t]]$ is unconstrained. Hence
\[
\Delta(Q, \theta) \;\cong\;
\begin{cases}
[\,0,\ \bar c\,] \;\times\; t\,\mathbb{R}[[t]]
   & \text{if } v(c) \geq 0 \text{ and } \bar c = \rho(c) \geq 0, \\[2pt]
\varnothing & \text{otherwise (i.e. } v(c) < 0 \text{ or } \rho(c) < 0).
\end{cases}
\]
\end{example}
