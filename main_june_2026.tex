...
\begin{definition}
Let $k$ be a field and let $m_k \subset k$ be a given fixed additive submonoid, that is $0_k \in m_k$ and $m_k$ is closed under addition. 
We say that an element $z \in k$ is non-negative if $z \in m_k$. In an abuse of notation, we denote it as $z \geq_k 0$ or as $z \geq 0$ if $k = \mathbb{R}$ and $m_k = \mathbb{R}_{\geq 0}$.
\end{definition}
\begin{example}
The standard theory on \cite{hille2003quivers} can be recovered by using $k = \mathbb{R}$ and $m_k = \mathbb{R}_{\geq 0}$. Other possible cases are either
$k =  F_p \left( \left(t \right) \right)$ with $m_k = F_p [[ t ]]$ or 
$k=\overline{\mathbb{F}}_p$ with $m_k = \mathbb{F}_p$.
\end{example}
\begin{definition}
...
\end{definition}
\begin{example}
  In the case $k=\mathbb{R}$ \cite{hille2003quivers}, it is known that $\Delta(Q, \theta)$ a convex polytope due to the fact that $Q$ is acyclic. Moreover, the faces of $\Delta(Q, \theta)$ correspond to certain spanning subquivers of $Q$.   
\end{example}
...
\begin{example}
Let $Q = (Q_0, Q_1)$ be the quiver defined by the vertex set $Q_0 = \{v_0, v_1\}$ and the arrow set $Q_1 = \{\alpha_1, \alpha_2\}$, where both arrows have source $v_0$ and target $v_1$. Next, we 
describe its Delta flow for an non negative weight $\theta = (-c,c)$ with $c$ in $m_k$.
The quiver $Q$ has two spanning trees:
$T_1 = (Q_0, \{\alpha_1\})$ and 
$T_2 = (Q_0, \{\alpha_1\})$. 
We consider a non-zero weight vector $\theta  \in \text{Map}(Q_0, k)$, so
$\theta(v_0) = -c$ and $\theta(v_1) = c$ for some non-zero element $c \in k$. 

In this case, each of the spanning trees $T_i$ supports a non-negative flow, that is  $\Delta(T_1, \theta)$ and $\Delta(T_2, \theta)$ are non-empty an equal to an $x_i$. Then, 
\begin{align*}
x_1 := \Delta(T_1, \theta) = (c,0)
&&
x_2 := \Delta(T_3, \theta) = (0,c)
&& 
c \in m_k
\end{align*}
Any flow
$x \in I_Q^{-1}(\theta)$ satisfies $x(\alpha_1) + x(\alpha_2) = c$. Therefore, 
\begin{align*}
I_Q^{-1}(\theta)
=
\left\{
x \in k^2
\; \Big| \;
x = \lambda_1x_1 + \lambda_2 x_2, \;
\lambda_i = x(\alpha_i)c^{-1}, \;
\lambda_1+\lambda_2 =1
\right\}
\end{align*}
By definition, the Delta flow  $\Delta(Q, \theta)$ is obtained by intersecting this affine line with the submonoid module $\text{Map}(Q_1, m_k) = m_k^2$.  Therefore, 
\begin{align*}
\Delta(Q,\theta)
=
\left\{
\lambda_1x_1 + \lambda_2 x_2,
\; \Big| \;
\lambda_ic \in m_k, \;
\lambda_1+\lambda_2 =1
\right\}
\end{align*}
In the classical case, $k = \mathbb{R}, m_k = \mathbb{R}_{\geq 0}$, we have $c >0$.
and the condition 
Since $c > 0$, the conditions $\lambda_ic >0$ forces $\lambda_i \geq 0$. This yield
\begin{align*}
 \Delta(Q,\theta)
=
\left\{
...
\right\}
\end{align*}   
On the other hand if $k = \mathbb{F}_2((t))$ and $m_k = \mathbb{F}_2[[t]]$, 
The conditions require $\lambda_1 c \in \mathbb{F}_2[[t]]$ and $\lambda_2 c \in \mathbb{F}_2[[t]]$. Because $\text{char}(k)=2$, we have $\lambda_2 = 1 + \lambda_1$, we have 
\begin{align*}
 \Delta(Q,\theta)
&=
\left\{
\lambda_1x_1 + (1+\lambda_1) x_2
\; \Big| \;
\lambda_i \geq 0, \;
\lambda_1+\lambda_2 =1
\right\}
...
\mathbb{F}_2[[t]]
\end{align*} 
In this case $\lambda_i$ maybe not be in $m_k$. For example, if $c =t$, then $\lambda_1 = t^{-1}$ is valid.
\end{example}
\begin{definition}
...
Recall for each spanning tree $T \subset Q$, $\Delta(T, \theta)$ is a singular point if it is nonempty, in which case it is a vertex of $\Delta(Q, \theta)$. Thus, upon varying $\theta$ between the inside and outside of $C_T$, the polytope $\Delta(Q, \theta)$ changes combinatorially. \footnote{Why? I imagine that if I am a triangle, then I can lose a vertex, but then to also get another one.}

Hille thus calls the supporting hyperplanes of each $C_T$ walls of $C_Q$. \footnote{I dont think it is true. Can you give the part of Hille's paper where this is defined}
When $\theta$ does not lie directly on any such wall of $C_Q$, the weight is called generic. For more information on the behavior of the walls and chambers, see \cite{hille2003quivers}.
