...
\]
We say that $z \in k$ is \emph{non-negative}, written $z \geq_k 0$ (or $z \geq 0$ when the valuation is understood), if $z \in m_k$, that is, if $v(z) \geq 0$.
\end{definition}
\begin{example}
For $k = \mathbb{F}_p((t))$, the \emph{$t$-adic valuation} $v\big(\sum_{i \geq i_0} a_i t^i\big) = \min\{i \mid a_i \neq 0\}$ has value group $\Gamma = \mathbb{Z}$ and valuation ring $m_k = \mathbb{F}_p[[t]]$. Likewise, for $k = \mathbb{Q}_p$ the $p$-adic valuation has $\Gamma = \mathbb{Z}$ and $m_k = \mathbb{Z}_p$. In each case $m_k$ is a ring, so $m_k = -m_k$: the order is carried by the value group $\Gamma$, not by $m_k$ itself.
\end{example}
\begin{remark}
The classical theory of \cite{hille2003quivers} is the parallel \emph{ordered-field} instance $k = \mathbb{R}$, $m_k = \mathbb{R}_{\geq 0}$. There $m_k$ is a \emph{pointed} monoid ($m_k \cap (-m_k) = \{0\}$) and $\geq_k$ is a genuine partial order, which is precisely what makes $\Delta(Q,\theta)$ a bounded convex polytope. A valuation ring is never pointed ($m_k = -m_k$), so the two settings are formally analogous but geometrically distinct: over a valued field $\Delta(Q,\theta)$ is a set of \emph{integral flows} rather than a polytope. We develop both in parallel and flag where they diverge.
\end{remark}
\begin{definition}
...
\end{definition}
\begin{example}
In the ordered-field case $k=\mathbb{R}$ \cite{hille2003quivers}, $\Delta(Q, \theta)$ is a convex polytope because $Q$ is acyclic, and its faces correspond to certain spanning subquivers of $Q$. Over a valued field, $I_Q^{-1}(\theta)$ is an affine subspace of $\map(Q_1, k)$ and $\Delta(Q,\theta) = I_Q^{-1}(\theta) \cap \map(Q_1, m_k)$ is its set of $\mathcal{O}_v$-integral flows. Since $m_k$ is a ring, $\Delta(Q,\theta)$ is either empty or a coset of the $\mathcal{O}_v$-module $\ker I_Q \cap \map(Q_1, m_k)$; it has no vertices or faces, and the combinatorial polytope structure is replaced by this module of integral flows.
\end{example}
...
\begin{example}\label{ex:two-arrow}
...
The quiver $Q$ has two spanning trees, $T_1 = (Q_0, \{\alpha_1\})$ and $T_2 = (Q_0, \{\alpha_2\})$. Each supports a unique flow with weight $\theta$, namely
\begin{align*}
...
\right\}
\end{align*}
is an affine line in $k^2$. The Delta flow $\Delta(Q, \theta)$ is its intersection with $\map(Q_1, m_k) = m_k^2$:
\begin{align*}
\Delta(Q,\theta)
=
\left\{
...
\emph{Ordered-field case} ($k = \mathbb{R}$, $m_k = \mathbb{R}_{\geq 0}$, $c > 0$): the conditions $\lambda_i c \in m_k$ read $\lambda_i \geq 0$, so
\begin{align*}
 \Delta(Q,\theta)
=
\left\{
...
\lambda_1+\lambda_2 =1
\right\}
=\text{ConvexHull}\left\{ (c,0), (0,c)
\right\},
\end{align*}
a segment with vertices $x_1, x_2$.

\emph{Valued-field case} ($k = \mathbb{F}_2((t))$, $m_k = \mathbb{F}_2[[t]]$, $v$ the $t$-adic valuation): writing $u := x(\alpha_1)$, a flow on $Q$ is $(u, c-u)$, and
\begin{align*}
 \Delta(Q,\theta)
=
\left\{
...
\right\}.
\end{align*}
Because $m_k$ is a ring, $u \in m_k$ and $c-u \in m_k$ hold simultaneously if and only if $u \in m_k$ and $c \in m_k$. Hence
\begin{align*}
\Delta(Q,\theta) =
\begin{dcases}
\{(u, c-u) \mid u \in m_k\} \;\cong\; \mathcal{O}_v, & v(c) \geq 0,\\
\varnothing, & v(c) < 0.
\end{dcases}
\end{align*}
This dichotomy is exactly the test $\theta \in C_Q$, since $C_Q = I_Q(m_k^2) = \{(-s, s) \mid s \in m_k\}$ and $\theta = (-c,c) \in C_Q$ iff $c \in m_k$. For instance, if $c = t$ then $v(c) = 1 \geq 0$ and $\Delta(Q,\theta) \cong \mathbb{F}_2[[t]]$ is infinite; here the parameters $\lambda_i$ need not lie in $m_k$ (e.g.\ $u = 1$ gives $\lambda_1 = t^{-1} \notin m_k$). If $c = t^{-1}$ then $v(c) = -1 < 0$ and $\Delta(Q,\theta) = \varnothing$, since $u, c-u \in m_k$ would force $c = u + (c-u) \in m_k$.
\end{example}
\begin{definition}
...
\right\}
\]
The cone of weights $C_P \subset \map(Q_0, k)$ associated to $P$ is defined as
$C_P:= I_Q(D_p)$, and the polytope associated to $P$ is defined as
\[
\Delta(P, \theta) := I_Q^{-1}(\theta) \cap D_P
\]
\end{definition}

In the ordered-field case, for each spanning tree $T \subset Q$ the set $\Delta(T, \theta)$ is a single point when nonempty, and is then a vertex of $\Delta(Q, \theta)$. As $\theta$ crosses the boundary of $C_T$, the polytope $\Delta(Q, \theta)$ changes combinatorially; the cones $C_T$ refine $C_Q$ into a fan, whose codimension-one faces are the \emph{walls} of $C_Q$, and a weight off every wall is called \emph{generic} (see \cite{hille2003quivers}).
