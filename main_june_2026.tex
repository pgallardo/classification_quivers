...
Let $k$ be a field and $m_k \subseteq k$ a \emph{sub-semiring}, that is, a subset satisfying $0, 1 \in m_k$ and closed under addition and multiplication. Since $m_k \subseteq k$ and $k$ is a field, cancellativity is automatic. We call $(k, m_k)$ an \emph{admissible pair}, and say that $z \in k$ is \emph{non-negative}, written $z \geq_k 0$, if $z \in m_k$.
...
\begin{definition}
A \emph{weight} of $Q$ is a map $\theta \in \map(Q_0, k)$. A \emph{flow} on $Q$ is a map $x \in \map(Q_1, k)$. The \emph{incidence map} $I_Q : \map(Q_1, k) \longrightarrow \map(Q_0, k)$ is defined by
\[
...
\begin{definition}
For an admissible pair $(k, m_k)$, a quiver $Q$, and a weight $\theta \in \map(Q_0, k)$, the \emph{delta flow space} associated to $Q$ and $\theta$ is
\[
...
\]
The \emph{weight cone} of $P$ is $C_P := I_Q(D_P) \subseteq \map(Q_0, k)$, and the \emph{delta flow} of $P$ is $\Delta(P, \theta) := I_Q^{-1}(\theta) \cap D_P$.
\end{definition}

\begin{lemma}[Translation action]\label{lem:translation}
The $m_k$-submodule $\mathcal{K}_Q := \ker I_Q \cap \map(Q_1, m_k)$ acts on $\Delta(Q, \theta)$ by translation: if $x \in \Delta(Q, \theta)$ and $y \in \mathcal{K}_Q$, then $x + y \in \Delta(Q, \theta)$. Consequently, $\Delta(Q, \theta)$ is either empty or a torsor for $\mathcal{K}_Q$.
\end{lemma}

\begin{proof}
Since $m_k$ is closed under addition, $x(\alpha) + y(\alpha) \in m_k$ for all $\alpha \in Q_1$. Since $I_Q$ is linear, $I_Q(x + y) = I_Q(x) + I_Q(y) = \theta + 0 = \theta$. Hence $x + y \in \Delta(Q, \theta)$.
\end{proof}

\begin{remark}
By definition, $\theta \in C_Q := I_Q(\map(Q_1, m_k))$ if and only if $\Delta(Q, \theta) \neq \varnothing$. This emptiness criterion holds for any admissible pair $(k, m_k)$.
\end{remark}

\subsection{The ordered and valued cases}\label{sec:two-cases}

We distinguish two families of admissible pairs by isolating the axiom that governs the geometry of $\Delta(Q, \theta)$.

\begin{definition}
We say $(k, m_k)$ satisfies axiom \emph{(P)} if $m_k$ is \emph{pointed}, that is, $m_k \cap (-m_k) = \{0\}$.
\end{definition}

The two cases of interest are:

\begin{itemize}
    \item \textbf{Ordered case.} Let $k$ be an ordered field and $m_k = k_{\geq 0} := \{z \in k : z \geq 0\}$. Then $m_k$ is a pointed sub-semiring satisfying (P), and $m_k - m_k = k$. The standard instance is $k = \mathbb{R}$, $m_k = \mathbb{R}_{\geq 0}$.

    \item \textbf{Valued case.} Let $k$ be a field equipped with a valuation $v : k^\times \to \Gamma$ (a surjective map onto a totally ordered abelian group $(\Gamma, +, \geq)$ satisfying $v(xy) = v(x)+v(y)$ and $v(x+y) \geq \min\{v(x),v(y)\}$, extended by $v(0) := \infty$). Set $m_k := \mathcal{O}_v = \{z \in k : v(z) \geq 0\}$, the \emph{valuation ring} of $(k,v)$. Since $-1 \in \mathcal{O}_v$, axiom (P) fails: $m_k \cap (-m_k) = \mathcal{O}_v \neq \{0\}$. Standard instances are $k = \mathbb{F}_p((t))$ with the $t$-adic valuation ($m_k = \mathbb{F}_p[[t]]$) and $k = \mathbb{Q}_p$ with the $p$-adic valuation ($m_k = \mathbb{Z}_p$).
\end{itemize}

\begin{remark}
Axiom (P) is precisely what makes $\Delta(Q,\theta)$ bounded in the ordered case. For an acyclic quiver $Q$, any $y \in \ker I_Q \cap m_k^{Q_1}$ satisfying (P) must be zero (a nonzero non-negative flow on an acyclic quiver cannot have zero net flux at every vertex), so $\mathcal{K}_Q = \{0\}$ and $\Delta(Q,\theta)$ is a convex polytope by Lemma~\ref{lem:translation} \cite{hille2003quivers}. In the valued case, axiom (P) fails and $\mathcal{K}_Q \cong \mathcal{O}_v^{b_1(Q)}$, so a nonempty $\Delta(Q,\theta)$ is infinite whenever $b_1(Q) \geq 1$. The two theories diverge exactly along axiom (P).
\end{remark}

We now compute $\Delta(Q, \theta)$ in both cases for the two fundamental quivers.

\begin{example}[Tree case]\label{tree}
Suppose $Q$ is a tree, so $b_1(Q) = 0$, $\ker I_Q = 0$, and $I_Q$ is injective. Then $\mathcal{K}_Q = \{0\}$ regardless of $(k, m_k)$, and for any $\theta \in \operatorname{Im}(I_Q)$ there is a unique flow $x_\theta := I_Q^{-1}(\theta) \in \map(Q_1, k)$. Thus:
\begin{itemize}
    \item \textbf{Ordered case:} $\Delta(Q,\theta) = \{x_\theta\}$ if $x_\theta(\alpha) \geq 0$ for all $\alpha \in Q_1$, and $\varnothing$ otherwise.
    \item \textbf{Valued case:} $\Delta(Q,\theta) = \{x_\theta\}$ if $v(x_\theta(\alpha)) \geq 0$ for all $\alpha \in Q_1$, and $\varnothing$ otherwise.
\end{itemize}
In both cases $\Delta(Q,\theta)$ is either a single point or empty. The tree case is \emph{model-independent}: it is the injectivity of $I_Q$, not the geometry of $m_k$, that forces this dichotomy.
\end{example}

\begin{example}[Two-vertex quiver]\label{ex:two-arrow}
Let $Q$ be the quiver with $Q_0 = \{v_0, v_1\}$ and $Q_1 = \{\alpha_1, \alpha_2\}$, both arrows directed from $v_0$ to $v_1$. Fix $c \in k^\times$ and weight $\theta(v_0) = -c$, $\theta(v_1) = c$.

The two spanning trees $T_1 = (Q_0, \{\alpha_1\})$ and $T_2 = (Q_0, \{\alpha_2\})$ give unique flows $x_1 = (c, 0)$ and $x_2 = (0, c)$. Any flow in $I_Q^{-1}(\theta)$ satisfies $x(\alpha_1) + x(\alpha_2) = c$, so writing $u := x(\alpha_1)$:
\[
\Delta(Q, \theta) \;=\; \bigl\{\,(u,\; c - u) \;\big|\; u \in m_k,\; c - u \in m_k \,\bigr\}.
\]

\textbf{Ordered case} ($k = \mathbb{R}$, $m_k = \mathbb{R}_{\geq 0}$): The conditions $u \geq 0$ and $c - u \geq 0$ force $0 \leq u \leq c$. Thus
\[
\Delta(Q, \theta) \;=\;
\begin{cases}
\operatorname{ConvexHull}\!\left\{(c,0),\,(0,c)\right\} & \text{if } c > 0, \\
\varnothing & \text{if } c < 0.
\end{cases}
\]
This is a compact line segment, a one-dimensional polytope. The module $\mathcal{K}_Q = \{0\}$ (consistent with (P) and acyclicity).

\textbf{Valued case} ($k = \mathbb{F}_2((t))$, $m_k = \mathbb{F}_2[[t]]$, $t$-adic valuation): Since $\mathcal{O}_v$ is a ring, $u \in \mathcal{O}_v$ and $c - u \in \mathcal{O}_v$ hold simultaneously if and only if $u \in \mathcal{O}_v$ and $c \in \mathcal{O}_v$. Hence
\[
\Delta(Q, \theta) \;\cong\;
\begin{cases}
\mathcal{O}_v & \text{if } v(c) \geq 0, \\
\varnothing & \text{if } v(c) < 0.
\end{cases}
\]
This is infinite when nonempty: the module $\mathcal{K}_Q = \ker I_Q \cap \mathcal{O}_v^2 = \{(u,-u) : u \in \mathcal{O}_v\} \cong \mathcal{O}_v$ acts freely by translation, reflecting the failure of (P).

In both cases the emptiness test is identical ($\theta \in C_Q \iff c \in m_k$), but the structure of the nonempty fiber diverges sharply: a bounded segment (ordered, axiom (P)) versus an infinite $\mathcal{O}_v$-torsor (valued, axiom (P) fails).
\end{example}

In the ordered-field case, for each spanning tree $T \subset Q$ the set $\Delta(T, \theta)$ is a singleton when nonempty and is a vertex of $\Delta(Q, \theta)$. As $\theta$ crosses the boundary of $C_T$, the polytope $\Delta(Q, \theta)$ changes combinatorially; the cones $C_T$ refine $C_Q$ into a fan whose codimension-one faces are the \emph{walls} of $C_Q$, and a weight off every wall is called \emph{generic} (see \cite{hille2003quivers}). Over a valued field the membership test $\theta \in C_P$ still governs emptiness of $\Delta(P, \theta)$, but the wall-and-chamber picture has no direct analogue: when nonempty, $\Delta(Q, \theta)$ is an infinite $\mathcal{O}_v$-torsor rather than a convex hull of tree points.
